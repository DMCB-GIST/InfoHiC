...
def get_cn(cn_file, chrom, start, end):
    cn=[1]
...
with tf.compat.v1.Session(config = config) as sess:
    ...
    cn_placeholder = graph.get_tensor_by_name("cn:0")
    ...
    with open(FLAGS.input, "r") as rdf:
        ...
        for line in rdf:
            ...
            for seq in run_seqs:
                hotseqs = []
                cn=get_cn(FLAGS.cn_file, chrom, run_starts[i], run_ends[i])
