...
flags.DEFINE_string('cn_file', None, 'cn file')
...
def get_hot_coded_seq(sequence):
    ...
    # initialise empty
    hotsequence = np.zeros((FLAGS.bp_context,4), dtype='int32')
...
def get_cn_seq(sequence, cn_file, chrom, start, end, rc_tag):
    cn_seq = np.zeros((len(sequence),4), dtype='int32')
    ls=len(sequence)
    with open(cn_file, "r") as f:
        for l in csv.reader(f, delimiter='\t'):
            if l[0] == chrom and not (int(l[2]) < start or int(l[1])-1 > end) :
                ...
                if rc_tag:
                    cn_seq[(ls-n_end):(ls-n_start),:] = sequence[(ls-n_end):(ls-n_start),:]*int(l[3])
                else:
                    cn_seq[n_start:n_end,:] = sequence[n_start:n_end,:]*int(l[3])
    return cn_seq
        ...
        for line in rdf:
            ...
            for seq in run_seqs:
                ...
                if FLAGS.rc_tag == "True":
                    seq = get_hot_coded_seq_rc(seq)  # hot encode
                    seq = get_cn_seq(seq, FLAGS.cn_file, chrom, run_starts[i], run_ends[i],1)
                else:
                    seq = get_hot_coded_seq(seq)
                    seq = get_cn_seq(seq, FLAGS.cn_file, chrom, run_starts[i], run_ends[i],0)
                hotseqs.append(seq)
                hotseqs = np.asarray(hotseqs)
	
                if len(seq) != FLAGS.bp_context:
