...
def get_hot_coded_seq(sequence):
    ...
    return hotsequence
def get_hot_coded_seq_rc(sequence):
    ls=len(sequence)
    hotsequence = np.zeros((len(sequence),4), dtype='int32')
    for i in range(len(sequence)):
        if sequence[i] == 'A':
...
def get_ascn_seq(seq1,seq2, ascn_file,chrom, start, end,rc_tag):
        ...
        with open(ascn_file, "r") as f:
                for l in csv.reader(f, delimiter='\t'):
                        if l[0] == chrom and not (int(l[2]) < start or int(l[1])-1 > end) :
                              n_start=max( int(l[1])-1 , start) - start
                              n_end=min( int(l[2]) , end) - start
                              if rc_tag == "True":
                                ascn_seq[0, (len(seq1)-n_end):(len(seq1)-1-n_start),:] = seq1[(len(seq1)-n_end):(len(seq1)-1-n_start),:]*int(l[3])
                                ascn_seq[1, (len(seq1)-n_end):(len(seq1)-1-n_start),:] = seq2[(len(seq1)-n_end):(len(seq1)-1-n_start),:]*int(l[4])
                              else:
                                ascn_seq[0, n_start:n_end,:] = seq1[n_start:n_end,:]*int(l[3])
                                ascn_seq[1, n_start:n_end,:] = seq2[n_start:n_end,:]*int(l[4])
        return ascn_seq
...
#                print("run start %s %e" % (run_starts[i],run_ends[i]))
                ...
                if FLAGS.rc_tag == "True":
                    hap1_seq= get_hot_coded_seq_rc(hap1_seq)
                    hap2_seq= get_hot_coded_seq_rc(hap2_seq)
                    hap_seq=get_ascn_seq(hap1_seq,hap2_seq, FLAGS.ascn_file,chrom, run_starts[i], run_ends[i],FLAGS.rc_tag)
                else:
                    hap1_seq= get_hot_coded_seq(hap1_seq)
                    hap2_seq= get_hot_coded_seq(hap2_seq)
                    hap_seq=get_ascn_seq(hap1_seq,hap2_seq, FLAGS.ascn_file,chrom, run_starts[i], run_ends[i],FLAGS.rc_tag)
