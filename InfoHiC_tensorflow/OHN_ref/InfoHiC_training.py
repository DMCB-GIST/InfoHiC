...
def get_chr_seq(whg_fasta, chromosome, rc=False, dtype = "uint8"):
...
def rc_seqs(batch_size, seq_length, lfs):
    rc_seq=np.zeros((batch_size, seq_length, 4), dtype='int')
    for i in range(batch_size):
        for j in range(seq_length):
            rc_seq[i,j,0] = lfs[i,seq_length-j-1,3]
            rc_seq[i,j,1] = lfs[i,seq_length-j-1,2]
            rc_seq[i,j,2] = lfs[i,seq_length-j-1,1]
            rc_seq[i,j,3] = lfs[i,seq_length-j-1,0]
    return(rc_seq)
...
def do_eval(sess,
    ...
    with open(lf) as f:
        ...
        for l in f:
            ...
            if rc_tag:
                feed_dict = {
                        seqs_placeholder: fill_seqs(geno[chr_number], geno_l[chr_number]-1-int(l[2]), geno_l[chr_number]-1-int(l[1]), FLAGS.batch_size, BP_CONTEXT, dtype=FLAGS.store_dtype),
                ...
                feed_dict = {
                      seqs_placeholder: fill_seqs(geno[chr_number], int(l[1]), int(l[2]), FLAGS.batch_size, BP_CONTEXT, dtype=FLAGS.store_dtype),
                      labels_placeholder: labels_test[test_step:(test_step+1),],
                      keep_prob_inner_placeholder: 1.0,
                      keep_prob_outer_placeholder: 1.0
                      }
    ...
    (cases, test_loss))
    print_val_loss.append('Num examples: %d Test Loss: %0.04f' % (cases, test_loss))
    ...
    while epoch < FLAGS.max_epoch:
        ...
        for i in range(1,24):
            if i ==23:
                ...
                chr_name="chr"+str(i)
            if rc_tag:
                tmp_chr_seq=get_chr_seq(FLAGS.whg_fasta, chr_name, True, "uint8")
                geno.append(tmp_chr_seq)
                geno_l.append(len(tmp_chr_seq))
            else:
                tmp_chr_seq=get_chr_seq(FLAGS.whg_fasta, chr_name, False, "uint8")
                geno.append(tmp_chr_seq)
                geno_l.append(len(tmp_chr_seq))
                print(i)
        ...
        with open(lf) as f:
            ...
            for l in f:
                ...
                if (step+1) % FLAGS.report_every == 0:
                    ...
                    print_step_loss.append('Step %d: loss = %.2f (%.3f sec)' % (step+1, loss_value,duration))
