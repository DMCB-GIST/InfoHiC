...
library("ggplot2")
t[t[,3]<0,3]=0
t[t[,3]>as.numeric(args[1]),3]=as.numeric(args[1])
...
g=ggplot(data = t, aes(x=V1, y=V2, fill=V3)) +
	geom_tile() + scale_fill_gradientn(colours=brewer.pal(9, 'YlOrRd')) + theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
	panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) + coord_equal()
