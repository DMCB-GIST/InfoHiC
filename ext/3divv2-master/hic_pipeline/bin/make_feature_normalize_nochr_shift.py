...
chrlist = [ str(i+1) for i in range(22)]
chrlist.append('X')
...
def make_Feature(chr_bamfile, coverage_dict, resolution, cis_savefile, trans_savefile, chrname):
	...
	for line in bam.fetch():
		...
		if start_chrnum < 25 and end_chrnum < 25 and line.reference_start-shift > 0 and line.next_reference_start-shift > 0:

			if start_chrnum == 23: start_chrnum = 'X'
			if start_chrnum == 24: start_chrnum = 'Y'
			pos1_chr=  str(start_chrnum)

			if end_chrnum == 23: end_chrnum = 'X'
			if end_chrnum == 24: end_chrnum = 'Y'
			pos2_chr=  str(end_chrnum)

			bin1=(int(line.reference_start)-shift)/resolution
			bin2=(int(line.next_reference_start)-shift)/resolution

			map_id = ".".join([pos1_chr, str(bin1 * resolution+shift), str((bin1+1)* resolution+shift)]) + "," + ".".join([pos2_chr, str(bin2 * resolution+shift), str((bin2+1)* resolution+shift)]) 
	...
	if not len(trans_contact) == 0:
		...
		if chrname == '10': o.write("frag1\tfrag2\tcov_frag1\tcov_frag2\tfreq\tdist\n")
		for contact in trans_contact:
			[bin1, bin2] = contact.split(',')
			if bin1.split(".")[0] == 'Y' or bin2.split(".")[0] == 'Y': pass
			else:
				cov1 = coverage_dict[bin1]
				cov2 = coverage_dict[bin2]
